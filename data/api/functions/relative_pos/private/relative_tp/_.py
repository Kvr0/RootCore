...
def Root(Axis):
    s = 'execute if score $Pos.{} Temporary matches ..-1 run function api:relative_pos/private/relative_tp/{}/m\n'.format(Axis,Axis.lower())\
        +'execute if score $Pos.{} Temporary matches 01.. run function api:relative_pos/private/relative_tp/{}/p'.format(Axis,Axis.lower())
    return s

def RelativeTP(Axis,Sign):
    lines = []
    for _i in range(0,16):
        i = 15-_i

        s = 'execute if score $Pos.{} Temporary matches {}{} at @s run tp @s ~{} ~{} ~{}\n'.format(Axis,Sign*2**i if Sign == 1 else '..',Sign*2**i if Sign == -1 else '..',Sign*2**i / 1000 if Axis == 'X' else '',Sign*2**i / 1000 if Axis == 'Y' else '',Sign*2**i / 1000 if Axis == 'Z' else '')\
...
#execute if score $Pos.X Temporary matches ..-1 run function api:relative_pos/private/relative_tp/x/m
#execute if score $Pos.X Temporary matches 01.. run function api:relative_pos/private/relative_tp/x/p
